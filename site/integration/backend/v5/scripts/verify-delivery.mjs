...
assert.equal(manifest.schema, "meshful-backend-delivery.v5");
...
for (const file of manifest.files) {
  assert.ok(file.path.startsWith("backend/v5/"));
...
assert.deepEqual((await walk("backend/v5")).sort(compare), [...declared].sort(compare));
...
if (process.argv.includes("--external")) {
  ...
  process.stdout.write("Current canonical, public v3 and retained v1 release pins verified.\n");
}
process.stdout.write(`Verified ${manifest.file_count} v5 files and frozen v4 predecessor.\n`);
