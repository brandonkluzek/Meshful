...
test("malformed local claim hydration fails with a stable recovery error and consumes no durable slot", {
  timeout: 120_000,
}, async (t) => {
  const library = await loadRetainedResolver();
  const engine = await createCanonicalEngine({
  ...
  const learner = await contextFor(service, "claim-invalid-hydration");
  ...
  await assert.rejects(service.claimLocalState(learner, {
    request_id: "claim:invalid-hydration",
    expected_revision: 0,
    source_id: "browser-state:invalid-hydration",
    catalog_ref: library.expectedCatalogPins[0].constructorCatalogRef,
    raw_state_json: rawState,
  }), code("INVALID_LOCAL_STATE"));

  const after = await service.getState(learner);
  assert.equal(after.durable_revision, 0);
  assert.equal(after.state_json, null);
  assert.equal(await createD1Repository(db).getReceipt(learner.principalId,
    "claim:invalid-hydration"), null);
});
