...
function errorStatus(error) {
  ...
  if (/^(STALE_|WRITER_|ACTIVE_SESSION|SESSION_NOT_ACTIVE|DECK_IN_ACTIVE_SESSION|CARD_MISMATCH|INVALID_SESSION_PHASE|CATALOG_BASE_UNAVAILABLE|IDEMPOTENCY_|LIBRARY_DEPENDENCY_|PREREQUISITE_NOT_SATISFIED$)/.test(error?.code ?? "")) return 409;
...
export function createLearnerHandler({ service, authenticate, authenticationFailureResponse,
  ...
  return async function handle(request) {
    ...
    try {
      ...
      if (request.method === "GET" && path === `${prefix}/state`) result = await service.getState(context);
      else if (request.method === "GET" && path === `${prefix}/writer-grant`) {
        requireThat(url.search === "", "INVALID_INPUT", "Writer status has no query parameters");
        result = await service.getWriterGrant(context);
      }
      else if (request.method === "POST" && path === `${prefix}/writer-grant`) {
        result = await service.mutateWriterGrant(context, await readJson(request, maxBodyBytes));
      }
      else if (request.method === "POST" && path === `${prefix}/writer-grant/validate`) {
        requireThat(request.headers.get("content-length") === null || request.headers.get("content-length") === "0",
          "INVALID_INPUT", "Writer validation has no request body");
        result = await service.validateWriterGrant(context, writerGrant(request));
      }
        ...
        result = await service.claimLocalState(context, await readJson(request, maxBodyBytes), writerGrant(request));
      }
...
function writerGrant(request, { required = true } = {}) {
  const encodedEpoch = request.headers.get("x-meshful-writer-epoch");
  const token = request.headers.get("x-meshful-writer-token");
  if (!required && encodedEpoch === null && token === null) return null;
  requireThat(encodedEpoch !== null && /^(?:0|[1-9][0-9]{0,15})$/.test(encodedEpoch)
    && Number.isSafeInteger(Number(encodedEpoch)) && Number(encodedEpoch) >= 1,
  "WRITER_GRANT_REQUIRED", "Acquire the active study-writer grant before changing learner state", 409);
  requireThat(token !== null && /^[a-f0-9]{64}$/.test(token),
    "WRITER_GRANT_REQUIRED", "A valid study-writer grant is required", 409);
  return { writerEpoch: Number(encodedEpoch), token };
}
