...
const NON_ANSWER_GRADE_SCHEMA = {
  ...
  properties: {
    ...
    expected_card_revision: { type: "integer", minimum: 1 },
    expected_session_revision: { type: "integer", minimum: 1 },
    attempt_kind: { enum: ["reveal", "skip"] },
    idempotency_key: { type: "string", minLength: 1, maxLength: 128 },
  },
  required: [
    "session_id", "card_id", "expected_card_revision",
    "expected_session_revision", "attempt_kind", "idempotency_key",
  ],
};
...
function validateNonAnswerGradeResult(result, args) {
  assertJson(result);
  exactKeys(result, [
    "review_id", "session_id", "card_id", "card_revision", "attempt_kind",
    "answer_revealed", "rating", "schedule", "reviewed_card", "session",
    "next_card", "receipt",
  ], [
    "review_id", "session_id", "card_id", "card_revision", "attempt_kind",
    "answer_revealed", "rating", "schedule", "session", "receipt",
  ], "non-answer grade result");
  requireThat(result.session_id === args.session_id
    && result.card_id === args.card_id
    && result.card_revision === args.expected_card_revision
    && result.attempt_kind === args.attempt_kind
    && result.answer_revealed === (args.attempt_kind === "reveal")
    && result.rating === "again",
  "ENGINE_INVARIANT", "Non-answer result differs from the committed learner action", 503);
  requireThat(args.attempt_kind === "reveal"
    ? result.reviewed_card?.card_id === args.card_id
    : !Object.hasOwn(result, "reviewed_card"),
  "ENGINE_INVARIANT", "Reveal/Skip definition exposure differs from the committed learner action", 503);
  requireThat(result.receipt?.operation === "submit_non_answer_grade"
    && result.receipt.idempotency_key === args.idempotency_key
    && result.receipt.replayed === false,
  "ENGINE_INVARIANT", "Non-answer receipt differs from the committed action", 503);
}

function validateSelectedCourseInstall(result, args, beforeDeckIds, store) {
  assertJson(result);
  const records = result?.installation?.decks;
  requireThat(Array.isArray(records) && records.length === 1,
    "ENGINE_INVARIANT", "Library Add must install only the selected course", 503);
  const record = records[0];
  requireThat(record?.catalog_deck_id === args.library_deck_id
    && String(record.catalog_version) === String(args.expected_catalog_version)
    && record.deck_id === result.deck?.id
    && record.already_installed === result.already_installed
    && result.visible_effect?.type === "deck_added"
    && result.visible_effect.deck_id === record.deck_id,
  "ENGINE_INVARIANT", "Library Add result differs from the selected course", 503);
  const afterDeckIds = Object.keys(store.getSnapshot().personalDecks);
  const added = afterDeckIds.filter((id) => !beforeDeckIds.has(id)).sort();
  const expectedAdded = result.already_installed ? [] : [record.deck_id];
  requireThat(stableJson(added) === stableJson(expectedAdded),
    "ENGINE_INVARIANT", "Library Add materialized an unselected course", 503);
}
...
export async function createCanonicalEngine({
  ...
  async function open(record, now, intent) {
    ...
    const createStore = () => {
      try {
        return createStudyStore(options);
      } catch (error) {
    ...
    return { resolved, storage, store: createStore(), createStore, constructorCatalogRef };
  }
  ...
  return Object.freeze({
    ...
    async transition(record, { operation, args, requestId, now }) {
      ...
      let { store } = opened;
      const before = ["submit_grade", "submit_non_answer_grade", "submit_self_grade"].includes(operation)
      ...
      if (operation === "set_deck_archived" && args.archived === true) {
        const snapshot = store.getSnapshot();
        const active = Object.values(snapshot.sessions).find((session) =>
          session.deckId === args.deck_id && session.status === "active");
        if (active) {
          // Archive is an account-management action, not a Study-writer action.
          // Stage the pause inside the same canonical outer action so one D1
          // command also remains one app revision and one public receipt.
          const staged = JSON.parse(priorJson);
          const session = staged.sessions?.[active.id];
          requireThat(session?.status === "active" && session.deckId === args.deck_id &&
            session.revision === active.revision && staged.activeSessionId === active.id,
          "ENGINE_INVARIANT", "The active Study session could not be staged for Archive", 503);
          session.status = "paused";
          session.updatedAt = now;
          session.revision += 1;
          staged.activeSessionId = null;
          staged.activity.push({
            type: "study_paused",
            deckId: session.deckId,
            sessionId: session.id,
            at: now,
          });
          if (staged.activity.length > 200) staged.activity.splice(0, staged.activity.length - 200);
          storage.setItem(STORAGE_KEY, JSON.stringify(staged));
          store = createStore();
        }
      }
      ...
      } else if (!["submit_non_answer_grade", "submit_self_grade"].includes(operation)) {
        validateSchema(schemas[operation].output, result, "result");
      }
