...
import { MAX_COMMAND_NODES, MAX_HTTP_BODY_BYTES } from "./capacity.mjs";
...
function errorStatus(error) {
  ...
  if (/^(STALE_|ACTIVE_SESSION|CARD_MISMATCH|INVALID_SESSION_PHASE|CATALOG_BASE_UNAVAILABLE|IDEMPOTENCY_|LIBRARY_DEPENDENCY_|PREREQUISITE_NOT_SATISFIED$)/.test(error?.code ?? "")) return 409;
...
function failure(error) {
  ...
  return jsonResponse({ ok: false, error: {
    ...
    message: error instanceof BackendError ? error.message : (status < 500
