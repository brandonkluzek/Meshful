...
Use all files in `backend/v4/FILE_MANIFEST.json`. The runtime entry is
`backend/v4/src/index.mjs`. Website should select `createCanonicalEngine` and
...
lease and trusted authenticator. The v4 handler changes only the owned error map
needed to preserve `DECK_IN_ACTIVE_SESSION` as nonretryable HTTP 409.

Configure the resolver with the public v2 exact-reference index as primary and
the private v1 exact-reference index as retained. Pass `expectedCatalogPins` in
that same `[v2, v1]` constructor order. V4 routes Library browse/preview/install
to v2 and promotes the saved constructor only in a successful install commit;
do not rewrite stored `libraryBase` values or catalog references in Website
code.

The browser should import `backend/v4/src/durable-client.mjs` directly, as it
...
the v4 server engine, capacity module, resolver or D1 repository.
...
silently relabeled as v4 evidence.
