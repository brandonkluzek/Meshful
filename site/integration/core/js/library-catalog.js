...
const PUBLIC_LIBRARY_CATALOG_REFS = Object.freeze([
  Object.freeze({
    version: "2026-09-03.public-sanitized.v4",
...
function requireResolverReleaseBoundaryPins(boundary, catalogRef, graphDigest, label) {
  ...
  requireThat(PUBLIC_LIBRARY_CATALOG_REFS.some((reference) => sameReference(catalogRef, reference))
...
export async function prepareLibraryCatalogResolver({
  ...
  async function resolveTransaction({ constructorCatalogRef, stateJson, intent } = {}) {
    ...
    if (["add_library_deck", "addLibraryDeck"].includes(name)) {
      ...
      const installationRoot = activeDescriptor.records.get(deckId);
      ...
      addRequest(activeDescriptor, installationRoot, "CATALOG_BASE_UNAVAILABLE");
    }
