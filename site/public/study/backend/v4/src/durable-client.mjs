...
export function createDurableClient({
  fetchImpl = globalThis.fetch, baseUrl = "/api/learner/v2", outbox,
} = {}) {
  ...
  async function sendPending() {
    ...
    try { data = await post(original); }
      ...
      if (isArchiveDurableRace(error, original)) {
        try {
      ...
      } else if (isDefiniteUncommittedArchiveRejection(error, original)) {
        settleDefiniteUncommitted(original);
        throw error;
      } else {
    ...
    if (requiresReplayReceipt && receipt.replayed !== true) {
      throw new BackendError("INVALID_SERVER_RESPONSE",
        "The Archive receipt lookup did not confirm a replay; keep and retry the exact recovery draft", 502);
    }
...
function isDefiniteUncommittedArchiveRejection(error, draft) {
  return draft?.command?.operation === "set_deck_archived"
    && error?.status >= 400
    && error?.status < 500
    && TERMINAL_UNCOMMITTED_ARCHIVE_CODES.has(error?.code);
}

function isArchiveDurableRace(error, draft) {
  return draft?.command?.operation === "set_deck_archived"
    && error?.code === "STALE_DURABLE_REVISION"
    && error?.status === 409;
}
