...
test("the generated server owns the mutually exclusive browser entry selection", async () => {
  ...
  assert.match(page, /const WEBSITE_ASSET_REVISION = 'v39-catalog-graph'/);
  ...
  assert.match(page, /ChatGPT sign-in identifies you, but study data does not sync between devices yet\./);
  assert.match(page, /Decks, reviews, and progress stay in this browser\./);
  assert.match(page, /Signing out does not clear data saved in this browser\./);
  assert.match(page, /Use a separate browser profile on a shared device\./);
  assert.match(page, /Data &amp; privacy/);
  assert.match(page, /Reset study data/);
  ...
  assert.equal((page.match(/>Library</g) ?? []).length, 2);
  assert.doesNotMatch(page, />Deck Library</);
  ...
  assert.match(app, /accountStorageState\.textContent = "Synced to your account"/);
  assert.match(app, /<h2 id="settings-title">Data &amp; privacy<\/h2>/);
  assert.match(app, /data-storage-state>Synced to your account/);
  ...
  assert.match(accountSettings, /Synced to your account/);
  assert.match(accountSettings, /<h3>Data deletion<\/h3>/);
  assert.match(accountSettings, /Permanent deletion<\/span><span>Not available yet/);
  assert.match(accountSettings, /Archiving a deck keeps its cards and study history\./);
  assert.match(accountSettings, /without affecting a later reinstall\./);
  assert.match(app, /Reset study data in this browser\? This removes locally saved personal decks, sessions, and review history\. Graph layout settings are not cleared by this reset\./);
...
test("the account constructor selects the sanitized public Deck Library while retaining prior exact bases", async () => {
  ...
  assert.match(accountStart, /account-entry\.js\?release=v39-catalog-graph/);
  assert.match(accountEntry, /js\/app\.js\?release=v39-catalog-graph/);
...
test("offline math styling is shipped locally and the correct initialization entry is available", async () => {
  ...
  assert.match(entry, /app\.js\?release=v39-catalog-graph/);
