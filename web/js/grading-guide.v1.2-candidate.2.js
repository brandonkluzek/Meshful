...
export const GRADING_GUIDE_VERSION = "definition-grading-guide.v1.2-candidate.2";
...
export const GRADING_GUIDE = [
  ...
  "FINALIZATION CHECK\nBefore finalizing: (1) compare every misconception with the learner's exact answer. Omitted meaning is not an asserted false belief; include only substantial false beliefs the answer actually expresses. (2) Include an optional compound legacy major-error ID only when the answer semantically asserts the whole named error, including every scope-changing modifier; partial phrase overlap is insufficient. If the answer expresses a different substantial conflict, record only that actual claim in an affected required row when one exists and in misconceptions/feedback, without attaching the unmatched legacy ID or changing unrelated rows. (3) For Again, when the answer contains meaningful support for a defining condition, briefly credit that support while giving the decisive gap and shortest correction. Do not manufacture praise for incidental overlap; keep feedback within the usual 1–3 sentences.",
