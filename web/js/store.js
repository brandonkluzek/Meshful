...
export function createStudyStore({ catalog, retainedCatalogs = [], storage, clock, timeZone } = {}) {
    ...
    const matches = [...catalogDecks.values()]
      .filter((deck) => {
        ...
        const haystack = [
          ...
          deck.subject,
          deck.level,
          ...deck.cards.slice(0, 30).map((card) => card.term),
        ]
          .join(" ")
          .toLowerCase();
  ...
  function addLibraryDeck(rawArgs = {}) {
    ...
    return withWrite("add_library_deck", args, (nextState) => {
      if (catalogDeck.libraryBase) {
        return installRequiredLibraryClosure(nextState, catalogDeck, catalogDecks, now().toISOString());
      }
      ...
      const personalId = uniqueDeckId(nextState, `deck-${slug(catalogDeck.id)}`);
  ...
  function startStudySession(rawArgs = {}) {
    ...
    return withWrite("start_study_session", args, (nextState) => {
      ...
      const queue = buildStudyQueue(deck, mode, limit, focusCardIds, now(), nextState, catalogDecks);
    ...
    for (const deck of selected) {
      ...
      for (const id of deck.cardOrder) {
        ...
        if (Number(card.review?.repetitions ?? 0) > 0) {
          ...
          else if (Number.isFinite(due) && (nextDue === null || due < nextDue)) nextDue = due;
      ...
      summaries.push({
        ...
        due_count: dueCount, eligible_new_count: ready.length, blocked_new_count: blocked.length,
      ...
      if (blockedLimit) {
        ...
        blockers = {
          ...
          next_cursor: offset + page.length < blocked.length
            ? `availability-v1:${state.revision}:${encodeURIComponent(deck.id)}:${offset + page.length}` : null,
        };
      }
    }
    return jsonClone({
      ...
      .filter((deck) => {
        ...
        if (
          ...
          !deck.cards.some((card) =>
            card.difficultyHint && difficultyHints.includes(card.difficultyHint.toLowerCase()),
          )
        ) return false;
        if (!query) return true;
        return [
  ...
  function startTargetStudySession(rawArgs = {}) {
    ...
    return withTargetWrite("start_study_session", args, (nextState, committedAt) => {
      ...
      const queue = buildStudyQueue(deck, "mixed", limit, [], new Date(committedAt), nextState, catalogDecks);
  ...
  function seedMasteredDemoDeck() {
    ...
    const canSeed = (snapshot) => {
      if (catalogDeck?.provenance !== "owner_commissioned_public_example" || !catalogDeck.cards.length) return false;
      ...
      return decks.some((deck) =>
        ...
        deck.source?.provenance === "owner_commissioned_public_example" &&
...
function catalogWithRetainedBases(input, retainedCatalogs) {
  ...
  for (const catalog of [catalogDecks, ...retainedCatalogs.map(normalizeCatalog)]) {
    for (const deck of catalog.values()) {
      ...
      if (existing && stableStringify(existing) !== stableStringify(deck)) {
        fail("INVALID_CATALOG", `Conflicting immutable catalog base ${deck.id} version ${deck.version}`);
      }
      bases.set(key, deck);
      if (deck.libraryBase) releases.set(stableStringify(deck.libraryBase.catalogRef), catalog);
    }
  }
...
function resolveCatalogBase(catalogDecks, reference) {
  ...
  return base && String(base.version) === String(reference.catalogVersion) ? base : null;
}

function normalizeCatalog(input) {
  const prepared = input?.kind === "meshful-library-runtime-catalog.v1";
  ...
  let rawDecks = [];
  if (prepared) rawDecks = input.catalog;
...
function libraryCardOwners(catalogDecks, savedDeck = null) {
  ...
  if (savedDeck?.libraryBase) {
    ...
    if (!sameLibraryBase(catalogDecks.get(savedDeck.libraryBase.catalogDeckId)?.libraryBase, savedDeck.libraryBase)) {
...
function installRequiredLibraryClosure(state, root, catalogDecks, installedAt) {
  const ids = root.requiredCatalogDeckIds;
  if (!Array.isArray(ids) || ids.at(-1) !== root.id || new Set(ids).size !== ids.length) {
    fail("LIBRARY_DEPENDENCY_MISSING", "The requested Library deck has no complete pinned dependency closure");
  }
  // Preflight every base and existing installation before materializing any
  // deck. withWrite owns the sole persist and includes this complete receipt.
  const plan = new Map();
  for (const catalogId of ids) {
    const base = catalogDecks.get(catalogId);
    if (!base?.libraryBase
      || stableStringify(base.libraryBase.catalogRef) !== stableStringify(root.libraryBase.catalogRef)
      || base.libraryBase.dependencyGraphDigest !== root.libraryBase.dependencyGraphDigest) {
      fail("LIBRARY_DEPENDENCY_MISSING", `Required Library base ${catalogId} is unavailable`);
    }
    const matches = matchingLibraryInstallations(state, catalogId);
    if (matches.length > 1 || (matches.length && !sameLibraryBase(matches[0].libraryBase, base.libraryBase))) {
      fail("LIBRARY_DEPENDENCY_CONFLICT", `Required Library base ${catalogId} conflicts with an existing installation`);
    }
    if (matches[0]?.archived) {
      fail("LIBRARY_DEPENDENCY_ARCHIVED", `Required Library deck ${catalogId} is archived; restore it explicitly`);
    }
    plan.set(catalogId, { base, existing: matches[0] ?? null });
  }
  const owners = libraryCardOwners(catalogDecks);
  for (const { base } of plan.values()) {
    for (const card of base.cards) {
      for (const parentId of card.prerequisiteIds ?? []) {
        const owner = owners.get(parentId);
        const required = owner && plan.get(owner.deck.id);
        if (!required) fail("LIBRARY_DEPENDENCY_MISSING", `Required parent ${parentId} has no pinned installation base`);
        if (!required.existing) continue;
        const parent = hasOwn(required.existing.cards, parentId) ? required.existing.cards[parentId] : null;
        if (!parent || parent.id !== parentId) {
          fail("LIBRARY_DEPENDENCY_CONFLICT", `Required canonical parent ${parentId} is missing from the installation`);
        }
        if (parent.archived) fail("LIBRARY_DEPENDENCY_ARCHIVED", `Required parent card ${parentId} is archived`);
      }
    }
  }
  const records = [];
  let requestedDeck;
  for (const [catalogId, entry] of plan) {
    const deck = entry.existing ?? personalDeckFromCatalog(
      entry.base, uniqueDeckId(state, `deck-${slug(catalogId)}`), installedAt,
    );
    if (!entry.existing) state.personalDecks[deck.id] = deck;
    records.push({
      catalog_deck_id: catalogId,
      catalog_version: entry.base.version,
      deck_id: deck.id,
      payload_sha256: entry.base.libraryBase.payloadDigest,
      normalized_digest: catalogBaseDigest(entry.base),
      already_installed: Boolean(entry.existing),
    });
    if (catalogId === root.id) requestedDeck = deck;
  }
  state.view.selectedDeckId = requestedDeck.id;
  if (records.some(record => !record.already_installed)) {
    recordActivity(state, {
      type: "deck_added", deckId: requestedDeck.id, at: installedAt,
      installedDeckIds: records.filter(record => !record.already_installed).map(record => record.deck_id),
    });
  }
  return {
    deck: personalDeckSummary(requestedDeck, new Date(installedAt)),
    already_installed: Boolean(plan.get(root.id).existing),
    visible_effect: { type: "deck_added", deck_id: requestedDeck.id },
    installation: {
...
function targetLibraryDeckSummary(deck) {
  const external = externalPrerequisitesForCatalog(deck);
  ...
  return {
    ...
    cross_deck_edge_count: external.length,
...
function targetCompleteDeck(deck, scope, at, catalogDecks) {
  ...
  const external = scope === "library"
    ? externalPrerequisitesForCatalog(deck)
    : externalPrerequisitesForPersonal(deck);
  ...
  const storedDeck = {
    ...
    version: scope === "library" ? String(deck.version) : String(deck.source?.catalogVersion ?? "personal"),
    title: deck.title,
    description: deck.description,
    subject: deck.subject,
    domain: deck.domain ?? deck.subject,
  ...
  return {
    ...
    cross_deck_edge_count: external.length,
    external_prerequisite_deck_ids: [...new Set(external.map((id) => deck.libraryBase
      ? libraryCardOwners(catalogDecks, deck).get(id)?.deck.id
      : String(id).split(".")[0]))].filter(Boolean),
...
function availabilityCursorOffset(raw, revision, deckId) {
  ...
  const match = /^availability-v1:(\d+):([^:]+):(\d+)$/.exec(value);
  if (!match) fail("INVALID_CURSOR", "Invalid blocked-card cursor");
  let cursorDeckId;
  try { cursorDeckId = decodeURIComponent(match[2]); }
...
function deckWarnings(deck) {
  ...
  if (externalPrerequisitesForPersonal(deck).length) {
    warnings.push(diagnostic("EXTERNAL_PREREQUISITES_UNVERIFIED", "deck.cards", "This deck check does not verify external prerequisite targets. Study treats each reference as required: an unresolved or archived parent, or a parent without prior Good/Easy recall, blocks first introduction."));
  }
  return warnings;
}
...
function libraryDetail(deck, state) {
  ...
  return {
    ...librarySummary(deck),
    tags: jsonClone(deck.tags),
    sample_terms: deck.cards.slice(0, 8).map((card) => card.term),
    graph: { acyclic: true, max_depth: graphMaxDepth(deck.cards.map((card) => card.id), deck.edges) },
...
function unmetStudyPrerequisite(deck, parentId, state, catalogDecks) {
  ...
  if (!parent && deck.libraryBase) {
    catalogOwner = libraryCardOwners(catalogDecks, deck).get(parentId) ?? null;
