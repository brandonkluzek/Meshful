...
import { encodeDocument } from "./fragment-codec.mjs";
...
export function createLearnerService({ repository, engine, clock = () => new Date() }) {
  ...
  async function commit(principalId, input, fingerprint, transition, baseRecord, importArchive) {
    ...
    try {
      outcome = await repository.commit({ principalId, expectedRevision: input.expected_revision,
        ...
        documents, stateDocumentId, responseDocumentId, baseRecord, events, importArchive: archive, now: now() });
    } catch (error) {
  ...
  return Object.freeze({
    ...
    async command(context, rawInput) {
      ...
      boundedArgs(rawInput.args); engine.validateCommand(rawInput.operation, rawInput.args, rawInput.request_id);
    ...
    async claimLocalState(context, rawInput) {
      ...
      return commit(principalId, input, fingerprint, transition, record, { sourceId: input.source_id, digest, rawJson: input.raw_state_json });
    },
